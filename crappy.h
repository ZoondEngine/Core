...
public:
	...
	template<typename T> T read(uint64_t addr)
	{
		...
		ReadProcessMemory(hProcess, (LPCVOID)addr, &buff, 0x8, NULL);
		return buff;
	}

	template<typename T> T read(SIZE_T address, DWORD bufSize)
	{
		T buff;
	...
	{
		return ReadProcessMemory(hProcess, (LPCVOID)src, dest, size, NULL);
	}
...
namespace Ptr
{
	...
	extern uint64_t level;
	extern uint64_t gameInstance;
	extern uint64_t localPlayer;
	extern uint64_t playerController;
	extern uint64_t playerCameraManager;
	extern uint64_t viewportClient;
	extern uint64_t pWorld;
	extern uint64_t actor;
	extern uint64_t actorList;
	extern uint32_t actorCount;
	extern uint64_t localPlayerActor;
...
{
	/*
	extern uint64_t PUWorld;
	extern uint64_t PLevel;
	extern uint64_t PGameInstance;
	extern uint64_t PLocalPlayer;
	extern uint64_t PUWorld;
	*/
